...
//objects within each panel
	...
	public void makeNorth()
	{
		...
		exitCurrentGame.addActionListener(new ActionListener()
		{
			...
			{
				callObject.sendMessageToServer("E");
			}
		});
		...
		public void actionPerformed(ActionEvent evt)
		{
			...
			if(!GameEnd)
			{
				if((playStatus == false) && (messageInput.isFocusOwner()==false))
				{
					...
					callObject.sendMessageToServer("G");
				}
			else
			{
				...
				String setSub = "S~";
					...
					callObject.sendMessageToServer(setSub);
					cardSelection.clear();
				}
			}
			}
		...
		public void actionPerformed(ActionEvent event){
			...
			if(messageInput.isFocusOwner()){
				...
				if(!userMessage.equals("")){
					...
					callObject.sendMessageToServer("T~"+userMessage);
				}
			}
			else {};
		}
		...
		default:
			...
			if(!(serverString[1].charAt(0) == 'N'))
			{
				...
				for(int i=0; i<cardsToShow.length;i++)
				{
					...
					JToggleButton setCard = new JToggleButton(new ImageIcon("src..." + cardsToShow[i] + ".gif"));
					...
					if(score != userScores[i])
					{
						if(username.equals(user))
						{
							...
							if(serverString[1].charAt(0) == 'Y')
							{
								...
								callObject.sendMessageToServer("M~" + username + " got 3 points!");
							}
							//wrong set
